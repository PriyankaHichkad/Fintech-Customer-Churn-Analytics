...
def simulate_retention_offers(df: pd.DataFrame, budget_cap: float = None) -> pd.DataFrame:
    ...
    df['cost_vip_perks'] = config.OFFER_COSTS['vip_perks_fixed']
    ...
    eff_apr_cut = np.where((df['max_delay_months'] > 0) | (df['current_utilization'] > 0.50),
                           np.minimum(eff_cfg['apr_cut']['max_reduction'], p_baseline * eff_cfg['apr_cut']['multiplier']), 0.0)
    ...
    # 4. Select Optimal Offer per Customer (Highest Net ROI > 0)
